...
import GenericLib.Constants;
 ...
 public class loginobjects extends Baseconfig {
	...
	// public static Object[][] FacilityWithMoreThanAllowedTime() throws
	 ...
	 * Thread.sleep(3000); List<WebElement> unredmail =loginobjects.
	 * Globalmethods.driver.findElements(By.xpath("//span[@class='zF']"));
	 * String data = "ItsMyAccount - Admin"; for (int i = 0; i <
	 * unredmail.size(); i++) { if(unredmail.get(i).isDisplayed() == true) { if
	 * (unredmail.get(i).getText().equals(data)) { String Actualsubjectinmail =
	 * unredmail.get(i).findElement(By.xpath("//span[@class='y2']")).getText();
	 * if (Actualsubjectinmail.equalsIgnoreCase(data)) { String userid =
	 * Actualsubjectinmail.split(" ")[7];
	 * unredmail.get(i).findElement(By.id("//span[@class='y2']")).click(); }
	 * else { System.out.println("no mail in inbox"); } break; } else {
	...
	// * unreademeil.size(); i++) { if (unreademeil.get(i).isDisplayed() ==
	// true)
	...
	// public String IdentifyUserpasswordInMailInbox() throws
	...
	// String SubjectInMail =
	// unreademeil.get(i).findElement(By.xpath("//*[@class='y6']/span/b"))
	// .getText();
	// String Subject = "Your credentials to access ItsMyAccount.com";
	// if (SubjectInMail.equals(Subject)) {
	// Reporter.log("Yes we have got mail form " + MyMailer + " With Subject Of
	// " + SubjectInMail,
	// true);
	...
	// } else {
	// Reporter.log("No mail form " + MyMailer + "With Subject Of " +
	// SubjectInMail, true);
	// }
	// break;
	// } else {
